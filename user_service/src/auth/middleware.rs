...
impl FromRequest for JwtMiddleware {
    ...
    fn from_request(req: &HttpRequest, _: &mut Payload) -> Self::Future {
        ...
        let auth_str = match auth_header.to_str() {
            Ok(s) => s,
            Err(_) => return ready(Err(ErrorUnauthorized("Incorrect format of authentication headline"))),
        };

        if !auth_str.starts_with("Bearer ") {
            return ready(Err(ErrorUnauthorized("The wrong authentication scheme is expected 'Bearer token'")));
        }
