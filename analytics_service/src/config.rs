...
impl Config {
    pub fn from_env() -> Self {
        dotenv().ok();
        Self {
            server_address: std::env::var("SERVER_ADDRESS").unwrap_or_else(|_| "127.0.0.1:8082".to_string()),
            mongodb_uri: std::env::var("MONGODB_URI").expect("MONGODB_URI must be set"),
            mongodb_db_name: std::env::var("MONGODB_DB_NAME").unwrap_or_else(|_| "analytics_db".to_string()),
